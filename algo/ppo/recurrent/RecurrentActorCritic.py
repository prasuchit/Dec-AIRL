...
class RecurrentActorCriticPolicy_Dec(ActorCriticPolicy_Dec):
    """
    ...
    def __init__(
        ...
        assert not (
            self.shared_lstm or self.enable_critic_lstm
        ), "We're using no LSTM for the critic at the moment. \
            We can add this feature in a future update."
        ...
        if not (self.shared_lstm or self.enable_critic_lstm):
            self.critic = nn.Linear(self.features_dim_critic, lstm_hidden_size)
    ...
    def forward(
        self,
        local_observation: th.as_tensor, 
        global_observation: th.as_tensor,
    ...
    def predict_values(
        ...
        """
        # # Call the method from the parent of the parent class
        # features = super(ActorCriticPolicy_Dec, self).extract_features(obs, self.vf_features_extractor)
        # if self.lstm_critic is not None:
    ...
    def predict(
        self,
        local_observation: Union[np.ndarray, Dict[str, np.ndarray]],
        ...
        observation, vectorized_env = self.obs_to_tensor(local_observation)
